...
sys.path.append("..")
from cutword import cut_word_via_statics
...
def cutInputImage(input_img_path, output_path, username):
    ...
    img_list = cut_word_via_statics(output_path, img, username, 0)
    return len(img_list)
...
def cutTrainImages(input_path, output_path, fonts_num, fonts_name):
    ...
    for font_name in fonts_name:
        ...
        for image in images:
            ...
            img = img.resize(IMAGE_SIZE)
            img_list = cut_word_via_statics(output_path, img, font_name, image_cnt)
